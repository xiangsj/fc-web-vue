  ...
  <div class="box">
    <div class="contentBox">
      <div class="operationBox">
        <div class="filter">
          <el-input v-model="searchValue" clearable placeholder="项目编号/项目名称"
                    style="height: 40px; width: 210px;"></el-input>
          <el-select v-model="Co_PIClassType" clearable placeholder="请选择项目类别"
                     style="height: 40px;width: 150px;margin-left: 20px;">
            <el-option
      ...
      <div class="tableBox">
        ...
        >
          ...
          <el-table-column prop="Pro_Code" label="项目编号" sortable width="180">
            <template slot-scope="scope">
              <router-link :to="{name:'detaiProject',params:{Proid:`${scope.row.Pro_GUID}`}}">
                {{scope.row.Pro_Code}}
              </router-link>
            </template>
          </el-table-column>
          <el-table-column prop="Pro_Name" label="项目名称" width="180"></el-table-column>
          <el-table-column prop="Pro_PIClassTypeName" label="项目类别" align="center"></el-table-column>
          <el-table-column prop="Pro_StatisticalTypeCodeName" label="统计分类" align="center"></el-table-column>
          <el-table-column prop="Pro_SalePrice" label="销售单价" align="center"></el-table-column>
          <el-table-column prop="Pro_StandardHour" label="准时工时" align="center"></el-table-column>
...
<script>
  ...
  import Delect from "./DelectProject"
    ...
    methods: {
      ...
      select() {
        ...
        this.$http.post(CommonDictionaryTree, this.$qs.stringify(selected)).then(resp => {
          for (let position in resp.data.data) {
            if (resp.data.data[position].Co_Code === 'AC03') {
      ...
      queryData(pageSize, currentPage,) {
        let Param = {
          ...
          Ba_Value: this.Co_PIClassType,
          State: this.State,
          Fendians: this.Fendians,
        };
        //获取接口数据
      ...
      //修改按钮
      updateProject(row) {
        this.$router.push({name: 'updateProject', params: {Guid: row.Pro_GUID}});
      },
      //添加按钮
      addProject() {
